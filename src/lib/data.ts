...
export interface AuditEvent {id:string;at:string;actor:string;action:string;reason:string;from?:CaseStatus;to?:CaseStatus}
export interface Evidence {id:string;name:string;type:string;status:'Tersedia'|'Kurang'|'Belum ada';updatedAt:string}
export interface ClaimCase {
  id:string;claimId:string;permitId:string;episodeId:string;prescriptionId?:string;participantToken:string;participantDisplayName:string;
  facilityId:string;facilityName:string;practitionerId:string;practitionerName:string;module:ModuleName;serviceDate:string;
  diagnosisCode:string;procedureCode:string;medicationCode:string;referralNumber:string;controlLetterNumber:string;
  evidenceStatus:'Lengkap'|'Sebagian'|'Tidak ada';riskScore:number;riskLevel:RiskLevel;reasonCodes:string[];ruleHits:string[];
  workflowStatus:CaseStatus;reviewer:string;createdAt:string;updatedAt:string;auditEvents:AuditEvent[];evidence:Evidence[]
}
